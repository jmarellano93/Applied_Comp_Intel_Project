...
Executes the genetic programming outer-loop across targeted activation functions.
Integrates strict generation-level checkpointing (every 10 generations), exact
stochastic seed resetting for mathematical fairness, environment documentation,
...
Methodological configuration:
    ...
    - Approximate Evaluations per Activation: 80,000
"""
...
class MOGPConfig(BaseModel):
    """Runtime configuration validated via Pydantic to ensure mathematical constraints."""
    population_size: int = Field(default=200, gt=0)
    ...
    def validate_probabilities(self) -> 'MOGPConfig':
        """Ensures probabilities do not exceed absolute bounds for eaMuPlusLambda."""
...
def get_system_environment(config: MOGPConfig) -> str:
    """
    Captures exact hardware and software dependencies for publication reproducibility.
...
def sanitize_sigma_squared(value: float) -> float:
    """Ensures variance strictly adheres to finite, positive, non-NaN constraints."""
...
def evaluate_rule(
    ...
    """Compiles the GP tree and tests its variance initialization against PyTorch models."""
...
def export_pareto_rules(
    ...
    """Exports structured results. Supports generation-level checkpointing to prevent data loss."""
    ...
    with output_path.open("w", encoding="utf-8") as file:
        ...
        file.write(f"Status: {'CHECKPOINT (Generation ' + str(gen) + ')' if is_checkpoint else 'FINAL COMPLETE RUN'}\n")
        file.write("=" * 72 + "\n")
...
def custom_eaMuPlusLambda(
    ...
    """
    Replaces DEAP's native algorithm block to explicitly inject a tqdm progress bar
    and generation-level checkpointing for long-running processes.
    """
    ...
    with tqdm(total=config.generations, desc=f"[{activation.upper()}] Evolutions", unit="gen") as pbar:
        for gen in range(1, config.generations + 1):
            ...
            # Checkpoint throttling: Export Pareto rules strictly every 10 generations
            if gen % 10 == 0:
...
def run_gp_for_activation(
    ...
    return export_pareto_rules(
        ...
        output_dir=output_dir, env_details=env_details, top_k=10, is_checkpoint=False
    )

def seed_runtime(seed: int) -> None:
    """Resets RNG strictly. Must be called before EVERY activation run for mathematical fairness."""
    random.seed(seed)
    np.random.seed(seed)
...
def main() -> None:
    ...
    print(f"Approx. Cumulative PyTorch FNN Evaluations: {total_evals:,}")

    exported_files = []

    for activation in config.activation_functions:
        # STRICT RESET: Ensures every activation starts from the exact same mathematical random state
