"""
Module 5: Optimization Method (OM) - MOGP Engine **PROTOTYPE**.

A scaled-down derivative of MOD6 used exclusively for pre-flight pipeline
validation BEFORE committing to the multi-day production sweep. The
mathematical machinery (NSGA-II, primitive set, protected operators,
consensus aggregation, Plan A/B toggle, resume safety, normalization
pre-flight assertion) is byte-identical to MOD6. Only three things differ:
    ...
    1. Scaled hyperparameters
       ...
       => ~9,720 PyTorch CPU evaluations for the full 18-pair x 3-seed sweep,
          vs MOD6's ~3.24M evaluations. Expect ~30 minutes to ~3 hours
          wall-clock depending on per-evaluation cost.

    2. Output routing
        Writes consensus artifacts to ``generated_files/GA_rule_files_testing/``
        (matches MOD7's default ``--rule_directory``), so the entire MOD7-9
        downstream pipeline can be exercised without CLI overrides.

    3. Status / label strings
        File headers say "MODULE 5 RESULTS" and the manifest is named
        ``MOD5_sweep_manifest.json`` to avoid being mistaken for the
        production output.

Purpose:
    Confirm end-to-end pipeline behaviour (MOD5 -> MOD7 -> MOD8 -> MOD9)
    on real (rather than synthetic) consensus artifacts BEFORE launching
    the production MOD6 run. If anything in the contract chain breaks,
    discover it in 30 minutes, not 7 days.
"""
...
def get_generated_directory() -> Path:
    ...
    generated_dir = module_dir / "generated_files" / "GA_rule_files_testing"
...
def export_pareto_rules(
    ...
    with output_path.open("w", encoding="utf-8") as file:
        file.write("APPLIED COMPUTATIONAL INTELLIGENCE: MODULE 5 RESULTS (PROTOTYPE)\n")
...
def main() -> None:
    ...
    print("\n--- Commencing Module 5 MOGP Meta-Grid Search (PROTOTYPE) ---")
    print(env_details)
    ...
    import json
    manifest_path = output_dir / "MOD5_sweep_manifest.json"
    with manifest_path.open("w", encoding="utf-8") as fh:
        json.dump(manifest, fh, indent=2)

    print("\n--- MODULE 5 MOGP META-GRID SEARCH COMPLETE (PROTOTYPE) ---")
