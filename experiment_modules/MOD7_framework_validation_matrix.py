"""
Module 7: Framework Validation Matrix.

Evaluates the top GP rules against six SOTA initialization baselines across
the Phase B validation dataset bench, then emits a per-activation JSON
...
Mathematical Notes:
    * Each per-trial training run uses Adam (lr=5e-3) with ``max_epochs=30``
      and balanced-accuracy early-stop at ``target_acc=0.85``.
    * Binomial p-value: one-tailed test of ``H_0: P(GP wins) = 0.5`` versus
      ``H_1: P(GP wins) > 0.5`` via ``1 - F(wins-1; trials, 0.5)``.
"""
...
def get_validation_reports_directory() -> Path:
    """Returns the absolute path of the MOD7-owned reports subfolder.

    Returns:
        Path under ``experimental_results_analysis_visualizations/reports/MOD7_validation_matrix/``.
    ...
    reports_dir = earv_dir / "reports" / "MOD7_validation_matrix"
...
def _select_phase_b_ids(
    manager: DatasetManager, quick_test: bool,
) -> List[int]:
    """Returns the dataset ids over which the validation sweep runs.

    Args:
        manager: A loaded ``DatasetManager``.
        quick_test: If True, returns only the first ``QUICK_TEST_DATASET_LIMIT`` ids.
    ...
    if quick_test:
        return all_ids[:QUICK_TEST_DATASET_LIMIT]
    return all_ids
...
def main() -> None:
    """CLI entry point. Parses args, runs the validation matrix, writes JSON."""
    parser = argparse.ArgumentParser(
        description="MOD7: Framework Validation Matrix (CPU)."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Collapse to 1 seed × 5 datasets for pipeline validation runs.",
    )
    ...
    trial_seeds = QUICK_TEST_SEED if args.quick_test else FULL_TRIAL_SEEDS

    print(f"\n--- STATISTICAL VALIDATION MATRIX [{args.activation.upper()}] ---")
    if args.quick_test:
        print("MODE: QUICK_TEST")
    ...
    dataset_ids = _select_phase_b_ids(manager, args.quick_test)
    ...
    # Build per-rule binomial p-values
