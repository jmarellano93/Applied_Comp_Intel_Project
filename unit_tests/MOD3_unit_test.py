...
def test_normalization_params_path_built(tmp_path):
    """The cache config exposes a norm_params_path under generated_files/."""
    cfg = mod3.CacheConfig(module_dir=str(tmp_path))
...
def test_load_normalization_params_applies_z_score(tmp_path):
    """
    When a normalization-params CSV exists, ``_load_normalization_params``
...
def test_load_normalization_params_graceful_fallback(tmp_path):
    """
    When the normalization params file is absent, the
    manager logs a warning and stays in raw mode (identity transform).
    """
    cfg = mod3.CacheConfig(module_dir=str(tmp_path))
