...
class TestActivationFilenameRegex:
    ...
    def test_extracts_activation_token(self, fname, expected):
        """The single-capture regex matches filenames without a topology token."""
...
class TestActivationTopologyFilenameRegex:
    """MOD6 consensus filenames embed both activation AND topology.
    ...
    @pytest.mark.parametrize("fname", [
        ...
        # A filename without a topology token must not match the new regex.
...
class TestSymbolicEngineSinCosRegression:
    """Rules containing sin/cos must parse without raising a NameError.
...
class TestMod9Paths:
    ...
    def test_rule_dir_default_matches_driver(self):
        ...
        config = mod9.QualitativeConfig()
        assert config.rule_dir.name == "GA_rule_files"
