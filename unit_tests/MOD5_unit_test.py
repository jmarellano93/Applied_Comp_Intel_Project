...
MOD6:
  ...
  * All MOD6 behaviours preserved (multi-seed replication, Plan B toggle, topology
...
providing a redundant safety net before launching the full production run.
"""
...
def test_get_generated_directory_routes_to_testing():
    ...
    artifacts with the MOD6 production output, which must be avoided.
    """
...
def test_total_evaluation_budget_is_prototype_scale():
    ...
    small enough for a fast end-to-end check, large enough to expose any
...
# =============================================================================
# CONFIGURATION CONTRACTS - must match MOD6 exactly
# =============================================================================

def test_qe_multi_seed_defaults():
    """GP discovery defaults to N=3 independent runs with seeds {42,43,44}."""
    config = mod5.MOGPConfig()
...
def test_qg_topology_targets_defaults():
    """The full sweep covers shallow, deep_narrow, and funnel."""
    config = mod5.MOGPConfig()
...
def test_force_rerun_default_preserves_resume_safety():
    ...
    pairs - undesirable both in MOD6 (it would overwrite production output) and in MOD5 (would
